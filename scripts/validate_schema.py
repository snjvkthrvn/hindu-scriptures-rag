...
from utils import CorpusValidator, VersValidator
...
class SchemaValidator:
    ...
    def print_results(cls, results: Dict[str, Any]) -> None:
        ...
        print(f"  Valid verses: {results['valid_verses']} ({results['valid_verses']/max(results['total_verses'],1)*100:.1f}%)")
        ...
        if results['coverage']:
            ...
            for text, cov in sorted(results['coverage'].items()):
                ...
                print(f"  {status_icon} {text}: {cov['actual']}/{cov['expected']} ({cov['percentage']:.1f}%)")

        if results['missing_texts']:
            print(f"\n⚠️  Missing/Incomplete Texts:")
            for missing in results['missing_texts']:
                print(f"  - {missing['text']}: {missing['actual']}/{missing['expected']} ({missing['missing']} verses missing)")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=700,
        help='Expected number of Gita verses'
    )
    parser.add_argument(
        '--check-all',
        action='store_true',
        help='Check all JSON files in final directory'
    )
