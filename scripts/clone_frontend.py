...
def clean_jinja_html(content, register_mode=False, beta_mode=False):
    ...
    content = re.sub(
        r"\{\{\s*url_for\('static',\s*filename='([^']+)'\)\s*\}\}", r"static/\1", content
    )
    content = re.sub(
        r"\{\{\s*url_for\('beta\.static',\s*filename='([^']+)'\)\s*\}\}", r"static/\1", content
    )
    ...
    content = re.sub(
        r"\{%\s*if\s+open_registration\s*%\}(.*?)\{%\s*endif\s*%\}", r"\1", content, flags=re.DOTALL
    )
    ...
    if "{% if register_mode %}" in content:
        if register_mode:
            ...
            content = re.sub(
                r"\{%\s*if\s+register_mode\s*%\}(.*?)\{%\s*else\s*%\}(.*?)\{%\s*endif\s*%\}",
                r"\1",
                content,
                flags=re.DOTALL,
            )
            content = re.sub(
                r"\{%\s*if\s+not\s+register_mode\s+and\s+open_registration\s*%\}(.*?)\{%\s*endif\s*%\}",
                "",
                content,
                flags=re.DOTALL,
            )
            content = re.sub(
                r"\{%\s*if\s+register_mode\s*%\}(.*?)\{%\s*endif\s*%\}",
                r"\1",
                content,
                flags=re.DOTALL,
            )
    ...
    content = re.sub(
        r"\{\{\s*url_for\('auth\.register',\s*next=next_url\)\s*\}\}", "register.html", content
    )
    content = re.sub(
        r"\{\{\s*url_for\('auth\.login',\s*next=next_url\)\s*\}\}", "login.html", content
    )
    ...
    content = re.sub(
        r"\{%\s*if\s+google_oauth_enabled\s*%\}(.*?)\{%\s*endif\s*%\}",
        r"\1",
        content,
        flags=re.DOTALL,
    )
    content = re.sub(
        r"\{\{\s*url_for\('auth\.google_login',\s*next=next_url\)\s*\}\}", "#google-login", content
    )
...
def clone_app(src_dir, dest_dir, is_beta=False):
    ...
    # Locate index.html
    index_path = (
        os.path.join(src_templates, "beta", "index.html")
        if is_beta
        else os.path.join(src_templates, "index.html")
    )
