...
from parsers import DharmicDataParser, UpanishadCSVParser, TextFileParser
from formatters import (
    SchemaNormalizer, process_directory as normalize_directory,
    MetadataEnricher, deduplicate_verses
)
from utils import CorpusValidator, VerseDetector
...
class PipelineOrchestrator:
    ...
    def export_metadata(self) -> None:
        ...
        for verse in verses:
            ...
            metadata['themes_used'].update(
                verse.get('metadata', {}).get('themes', [])
            )
            metadata['life_domains_used'].update(
                verse.get('metadata', {}).get('life_domains', [])
            )
