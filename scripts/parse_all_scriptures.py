...
def make_verse(verse_id, source_text, chapter, chapter_name, verse_num,
               sanskrit, translation='', transliteration='',
               category='shruti', tradition='vedic', themes=None,
               schools=None, section=None, commentaries=None,
               translations=None):
    ...
    for i in range(0, len(parts) - 1, 2):
        ...
        if verse_text and verse_num > 0:
            ...
            for line in lines:
                ...
                if verse_num == 1 and not clean_lines and not any(
                    c in line for c in '।॥'
                ) and len(line) < 80:
                    continue
    ...
    for chapter_file in sorted(gita_dir.glob('bhagavad_gita_chapter_*.json')):
        try:
            ...
            for verse_data in data.get('BhagavadGitaChapter', []):
                ...
                translation = ''
                for key in ['swami sivananda', 'shri purohit swami',
                            'swami gambirananda', 'dr. s. sankaranarayan']:
                ...
                verses.append(make_verse(
                    f"bg_{ch}_{v}", 'Bhagavad Gita', ch,
                    f"Chapter {ch}", v, text, translation,
                    category='smriti', tradition='vedanta',
...
def parse_rigveda(base_dir: Path) -> List[Dict[str, Any]]:
    ...
    for mandala_file in sorted(rig_dir.glob('rigveda_mandala_*.json')):
        try:
            with open(mandala_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            ...
            for sukta_data in data:
                ...
                for verse_num, verse_text in split_verses_by_marker(text):
                    verses.append(make_verse(
                        ...
                        'Rigveda', mandala, f"Mandala {mandala}",
                        verse_num, verse_text,
                        category='shruti', tradition='vedic',
...
def parse_atharvaveda(base_dir: Path) -> List[Dict[str, Any]]:
    ...
    for kaanda_file in sorted(av_dir.glob('atharvaveda_kaanda_*.json')):
        try:
            with open(kaanda_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            ...
            for sukta_data in data:
                ...
                for verse_num, verse_text in split_verses_by_marker(text):
                    verses.append(make_verse(
                        ...
                        'Atharvaveda', kaanda, f"Kaanda {kaanda}",
                        verse_num, verse_text,
                        category='shruti', tradition='vedic',
...
def parse_yajurveda(base_dir: Path) -> List[Dict[str, Any]]:
    ...
    for yv_file in sorted(yv_dir.glob('*.json')):
        try:
            with open(yv_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            ...
            for chapter_data in data:
                ...
                for verse_num, verse_text in split_verses_by_marker(text):
                    verses.append(make_verse(
                        ...
                        'Yajurveda', adhyaya, f"Adhyaya {adhyaya}",
                        verse_num, verse_text,
                        category='shruti', tradition='vedic',
...
def parse_mahabharata(base_dir: Path) -> List[Dict[str, Any]]:
    ...
    parva_names = {
        ...
        15: 'Ashramvasika Parva', 16: 'Mausala Parva',
        ...
        try:
            ...
            for entry in data:
                ...
                verses.append(make_verse(
                    ...
                    'Mahabharata', book, parva, shloka, text,
                    category='itihasa', tradition='common',
...
def parse_mahabharata_critical(base_dir: Path) -> List[Dict[str, Any]]:
    ...
    for mbh_file in sorted(ce_dir.glob('MBh*.json')):
        if mbh_file.name == 'MBh18UR.json':
            continue
        try:
            with open(mbh_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            ...
            for (book, chapter, shloka), parts in sorted(grouped.items()):
                ...
                verses.append(make_verse(
                    ...
                    'Mahabharata (Critical Edition)', book,
                    f"Book {book}", shloka, sanskrit,
                    transliteration=transliteration,
                    category='itihasa', tradition='common',
                    themes=['dharma', 'duty', 'morality'],
                    section=f"Chapter {chapter}"
...
def parse_valmiki_ramayana(base_dir: Path) -> List[Dict[str, Any]]:
    ...
    kanda_order = {
        'balakanda': 1, 'ayodhyakanda': 2, 'aranyakanda': 3,
        'kishkindhakanda': 4, 'sundarakanda': 5,
        'yudhhakanda': 6, 'uttarakanda': 7,
    }

    for rama_file in sorted(rama_dir.glob('*.json')):
        try:
            with open(rama_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            ...
            for entry in data:
                ...
                verses.append(make_verse(
                    ...
                    'Valmiki Ramayana', kanda_num, kanda_display,
                    shloka, text,
                    category='itihasa', tradition='common',
...
def parse_ramcharitmanas(base_dir: Path) -> List[Dict[str, Any]]:
    ...
    kanda_order = {
        'बालकाण्ड': 1, 'अयोध्याकाण्ड': 2, 'अरण्यकाण्ड': 3,
        'किष्किन्धाकाण्ड': 4, 'सुंदरकाण्ड': 5,
        'लंकाकाण्ड': 6, 'उत्तरकाण्ड': 7,
    }

    kanda_english = {
        'बालकाण्ड': 'Bala Kanda', 'अयोध्याकाण्ड': 'Ayodhya Kanda',
        'अरण्यकाण्ड': 'Aranya Kanda', 'किष्किन्धाकाण्ड': 'Kishkindha Kanda',
        'सुंदरकाण्ड': 'Sundara Kanda', 'लंकाकाण्ड': 'Lanka Kanda',
        'उत्तरकाण्ड': 'Uttara Kanda',
    }

    for rcm_file in sorted(rcm_dir.glob('*_data.json')):
        try:
            with open(rcm_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            ...
            for idx, entry in enumerate(data, 1):
                ...
                verses.append(make_verse(
                    ...
                    'Ramcharitmanas', kanda_num, kanda_en,
                    idx, content,
                    category='smriti', tradition='bhakti',
